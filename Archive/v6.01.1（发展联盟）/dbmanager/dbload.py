...
def dbInitialize():
    ...
    query = "SELECT * FROM raceCalendar ORDER BY Round ASC LIMIT 1;"
    ...
    query = "INSERT INTO driverList VALUES (%s, %s, %s, %s);"
    val = ("Race Director", "Reserve", group, "2022-01-01")
    cursor.execute(query, val)
    db.commit()

    # Initialize RaceDirector record
    query = "INSERT INTO raceDirector VALUES (%s, %s, %s, %s, %s, %s, %s);"
    val = ("C000", date, "Race Director", group, gpeng, 0, "prefix")
    cursor.execute(query, val)
    db.commit()
    

    # Initialize driver and constructor leaderboard and license point board
    query = "SELECT DISTINCT(Round), GP_CHN, GP_ENG FROM raceCalendar WHERE Round is not null LIMIT 5;" # hardcode number
    cursor.execute(query)
    result = cursor.fetchall()
