...
db = connectserver.connectserver(config, "afr")
cursor = db.cursor()
...
seasonname = servercfg["afr"].split("_")[1].upper()

# create and open the excel file
workbook = xlsxwriter.Workbook(f'AFR {seasonname}【{date_name}】{race_name}.xlsx')
...
def get_driverlist():
    ...
    for i in range(0,10):
        # A1
        query = f'SELECT driverList.driverName, teamList.teamColor, driverList.driverGroup, teamList.teamName \
                FROM driverList JOIN teamList ON \
                    driverList.team = teamList.teamName AND \
                    driverList.driverGroup = teamList.driverGroup \
                WHERE driverList.driverGroup = "A1" AND teamList.teamColor = "{teamname_ENG[i]}" \
                        AND driverList.driverName != "Race Dirctor" \
                ORDER BY driverList.driverStatus;'
        cursor.execute(query)
        result = cursor.fetchall()
        if len(result) > 0:
            teamname = result[0][3]
        ...
        for d in result:
            ...
            tempcursor += 1
        driverlist.write(a1row+2, a1col-1, teamname, formatter.driverformat[teamname_ENG[i]])
        a1row += 3
        ...
        query = f'SELECT driverList.driverName, teamList.teamColor, driverList.driverGroup, teamList.teamName \
                FROM driverList JOIN teamList ON \
                    driverList.team = teamList.teamName AND \
                    driverList.driverGroup = teamList.driverGroup \
                WHERE driverList.driverGroup = "A2" AND teamList.teamColor = "{teamname_ENG[i]}" \
                        AND driverList.driverName != "Race Dirctor" \
                ORDER BY driverList.driverStatus;'
        cursor.execute(query)
        result = cursor.fetchall()
        ...
        for d in result:
            ...
            tempcursor += 1
        driverlist.write(a2row+2, a2col-1, teamname, formatter.driverformat[teamname_ENG[i]])
        a2row +=3
...
def get_racecalendar():
    ...
    # Creating the header
    racecalendar.merge_range("A1:C1", "Champions League", formatter.racecalendar["RCHformat"])
    racecalendar.merge_range("E1:G1", "Challenger League", formatter.racecalendar["RCHformat"])
    racecalendar.merge_range("I1:K1", "Amateur League", formatter.racecalendar["RCHformat"])
...
def get_leaderboard_short():
    ...
    for i in range(0, len(group_list)):
        ...
        query = f'SELECT driverLeaderBoard.driverName, driverLeaderBoard.totalPoints, driverLeaderBoard.team, teamList.teamColor, \
                ...
                FROM driverLeaderBoard JOIN licensePoint ON \
                    driverLeaderBoard.driverName = licensePoint.driverName \
                                        LEFT JOIN teamList ON \
                    driverLeaderBoard.team = teamList.teamName AND \
                    driverLeaderBoard.driverGroup = teamList.driverGroup \
                WHERE driverLeaderBoard.driverGroup = "{group}" AND driverLeaderBoard.team != "Testing" \
        ...
        query = f'SELECT team, teamColor, totalPoints \
                FROM constructorsLeaderBoard JOIN teamList ON \
                        constructorsLeaderBoard.team = teamList.teamName AND \
                        constructorsLeaderBoard.driverGroup = teamList.driverGroup \
...
def get_leaderboard_constructors():
    ...
    for i in range(0, len(group_list)):
        ...
        for team in result:
            ...
            tempcursor = col
            # team name
            leaderboard.write(row, tempcursor+1, teamname, formatter.teamformat[teamcolor])
            ...
            leaderboard.write(row, 2 + race_count*2, team[-2], formatter.pointsformat["header"])
            row += 1
...
def get_leaderboard_full():
    ...
    for i in range(0, len(group_list)):
        ...
        query = f'SELECT driverLeaderBoard.*, teamList.teamColor \
                FROM driverLeaderBoard LEFT JOIN teamList ON \
                    driverLeaderBoard.team = teamList.teamName AND \
                    driverLeaderBoard.driverGroup = teamList.driverGroup \
                WHERE driverLeaderBoard.driverGroup = "{group}" AND driverLeaderBoard.team != "Testing" \
                    ORDER BY totalPoints DESC;'
        ...
        for driver in result:
            ...
            if team == "Retired":
                ...
                try:
                    leaderboard.write(row, tempcursor, drivername, formatter.driverformat[teamcolor])
                except:
            ...
            leaderboard.write(row, race_count+2, driver[-2], formatter.pointsformat["header"])
...
def get_lanusername():
    # setting row width
    cursor.execute("SELECT COUNT(*) FROM afr_elo.LANusername;")
...
def get_seasonstats():
    ...
    #  WHERE team != "Retired"
    query = 'SELECT driverList.*, teamList.teamColor FROM driverList \
                LEFT JOIN teamList ON driverList.team = teamList.teamName \
            ORDER BY CASE driverList.driverGroup \
                        WHEN "A1" THEN 1 \
                        WHEN "A2" THEN 2 \
                        WHEN "A3" THEN 3 \
    ...
    for driver in result:
        ...
        if driverteam == "Reserve" and drivergroup != "A3":
            ...
            try:
                ...
                seasonstats.write(row, col, drivername, formatter.driverformat[driver[-1]])
        col += 1
