...
workbook = xlsxwriter.Workbook(f'AFR {seasonname} 数据分析（R{round} {race_name}）.xlsx')
...
def get_raceresulttable():
    ...
    for raceresult in raceresult_list:
        ...
        # A1 group
        raceresult.merge_range("A2:F2", "Champions League", formatter.raceresultformat["a1headerf"])
        ...
        query = f'SELECT position, driverName, team, fastestLap, tyre, driverStatus, teamList.teamColor \
                FROM qualiResult LEFT JOIN teamList ON qualiResult.team = teamList.teamName \
                WHERE GP = "{therace}" and qualiResult.driverGroup = "A1";'
        cursor.execute(query)
        result = cursor.fetchall()
        ...
        for lap in result:
            try:
                ...
                try:
                    ...
                    raceresult.write(tempcursor, a1col+1, lap[1], formatter.driverformat[lap[-1]])
        ...
        # A2 group
        raceresult.merge_range("G2:L2", "Challenger League", formatter.raceresultformat["a2headerf"])
        ...
        query = f'SELECT position, driverName, team, fastestLap, tyre, driverStatus, teamList.teamColor \
                FROM qualiResult LEFT JOIN teamList ON qualiResult.team = teamList.teamName \
                WHERE GP = "{therace}" and qualiResult.driverGroup = "A2";'
        cursor.execute(query)
        result = cursor.fetchall()
        ...
        for lap in result:
            try:
                ...
                try:
                    raceresult.write(tempcursor, a2col+1, lap[1], formatter.driverformat[lap[-1]])
                except KeyError:
        ...
        #"""
        # A3 group
        raceresult.merge_range("M2:R2", "Amateur League", formatter.raceresultformat["a3headerf"])
        ...
        a1col = 0
        raceresult.merge_range(a1row, a1col, a1row, a1col+5, "Champions League", formatter.raceresultformat["a1headerf"])
        a1row += 1
        ...
        query = f'SELECT raceResult.*, teamList.teamColor FROM raceResult \
                    LEFT JOIN teamList ON raceResult.team = teamList.teamName \
                WHERE GP = "{therace}" and raceResult.driverGroup = "A1";'
        cursor.execute(query)
        result = cursor.fetchall()
        ...
        try:
            ...
            for position in result:
                ...
                if p[8] == "FINISHED":
                    ...
                    raceresult.write(tempcursor, a1col, "RET", formatter.raceresultformat[p[8]])
                else:
                    raceresult.write(tempcursor, a1col, p[7], formatter.raceresultformat[p[8]])

                try:
                    raceresult.write(tempcursor, a1col+1, p[3], formatter.driverformat[p[4]])
                except KeyError:
                    raceresult.write(tempcursor, a1col+1, p[3], formatter.driverformat[p[-1]])
            ...
            tempcursor += 1
            query = f'SELECT qualiraceFL.raceFLdriver, teamList.teamColor, \
                    ...
                    ON qualiraceFL.raceFLdriver = raceResult.driverName \
                                     LEFT JOIN teamList \
                    ON qualiraceFL.raceFLteam = teamList.teamName \
            ...
            if len(result) != 0:
                ...
                try:
                    ...
                    raceresult.write(tempcursor, a1col+1, fldriver, formatter.driverformat["Reserve"])
        ...
        a2col = 6
        raceresult.merge_range(a2row, a2col, a2row, a2col+5, "Challenger League", formatter.raceresultformat["a2headerf"])
        a2row += 1
        ...
        query = f'SELECT raceResult.*, teamList.teamColor FROM raceResult \
                    LEFT JOIN teamList ON raceResult.team = teamList.teamName \
                WHERE GP = "{therace}" and raceResult.driverGroup = "A2";'
        cursor.execute(query)
        result = cursor.fetchall()
        ...
        try:
            ...
            for position in result:
                ...
                if p[8] == "FINISHED":
                    ...
                    raceresult.write(tempcursor, a2col, "RET", formatter.raceresultformat[p[8]])
                else:
                    raceresult.write(tempcursor, a2col, p[7], formatter.raceresultformat[p[8]])

                try:
                    raceresult.write(tempcursor, a2col+1, p[3], formatter.driverformat[p[4]])
                except KeyError:
                    raceresult.write(tempcursor, a2col+1, p[3], formatter.driverformat[p[-1]])
            ...
            if len(result) != 0:
                ...
                try:
                    ...
                    raceresult.write(tempcursor, a2col+1, fldriver, formatter.driverformat["Reserve"])
        ...
        a3col = 12
        raceresult.merge_range(a3row, a3col, a3row, a3col+5, "Amateur League", formatter.raceresultformat["a3headerf"])
        a3row += 1
        ...
        raceresult.merge_range("S2:W2", "Weekend Overall", formatter.raceresultformat["a3headerf"])
        raceresult.write(2, 13, "车手", formatter.raceresultformat["headerf"])
        raceresult.write(2, 14, "圈速", formatter.raceresultformat["headerf"])
        raceresult.write(2, 15, "轮胎", formatter.raceresultformat["headerf"])
        srow = 3
